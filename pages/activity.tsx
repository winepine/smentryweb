import { Box, Container, Heading, HStack, Input, Select, Stack } from "@chakra-ui/react";
...
const Activity = () => {
  ...
  useEffect(()=>{
    ...
    if(searchValue!=''){  
      if(constraint=='house'){
        newData = newData.filter((act)=>act.house_no.house.includes(searchValue));
      }else{
        newData = newData.filter((act)=>act[constraint].includes(searchValue));
      }
    }
      setActivity(newData)
  ...
  <Stack p={8} px={16}>
    ...
    <HStack pt={8}>
      <Input minW={'85%'} value={searchValue} onChange={({target})=>setsearchValue(target.value)} bg="gray.100" placeholder="Search" />
      <Select onChange={({target})=>setConstraint(target.value)}>
        {
          constraints.map(con=>
            <option value={con}>{con[0].toLocaleUpperCase()+con.substring(1,con.length)}</option>
            )
        }
      </Select>
    </HStack>
